...
class SwingPong:
            ...
            if center[0] > self.x_center:
                if center[1] < self.y_center:
                    ...
                    if center[1] < self.y_start_threshold:
                        ...
                        self.game_start_tracking_flag = True
    ...
    def cvFrameReleaseAndDestroy(self, frame, vs):
    ...
    def StartTracking(self, center):
        ...
        if center < self.y_start_threshold and self.start_time == 0 and self.game_start_tracking_flag:
            self.start_time = time.time()
            print(self.start_time)

        # If condition is not true anymore:
        elif center > self.y_start_threshold and not self.game_start_tracking_flag:
            # Reset accumulator and start_time to defaults:
            self.accumulator = 0
            self.start_time = 0

        elif center < self.y_start_threshold and self.start_time != 0 and self.game_start_tracking_flag:
            self.accumulator = time.time() - self.start_time
            print("Accumulator: ", self.accumulator)
            if self.accumulator > 3:
                self.accumulator = 0
                self.start_time = 0
                self.game_start_tracking_flag = False
                self.game_start_flag = True
                self.StartGame(center)
                
                # print("Game started")
                # self.game_start_trigger = True
                # self.first_run = True
                # self.accumulator = 0
                # self.start_time = 0
